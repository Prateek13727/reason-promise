...
let fetchAllData = () => {
  ...
  ->Promise.flatMapOk(json => {
      ...
      data->DummyRepo.fetchCountries->Promise.mapOk(json => (json, data));
    })
  ->Promise.flatMapOk(((json, data)) => {
      let {countries}: Countries.t =
        json->Countries.t_decode->Utils.resolveResult("countries");
      let countryIds =
        countries->Belt.List.map(({id}: Countries.country) => id);
      countryIds
      ->DummyRepo.fetchPlaces
      ->Promise.mapOk(json => (json, data, countries));
    })
  ->Promise.mapOk(((json, data, countries)) => {
      let {places}: Places.t =
